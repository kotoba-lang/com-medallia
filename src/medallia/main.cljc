(ns medallia.main
  "Kotodama WASM entrypoint for the Medallia clean-room actor (L4) — Clojure port.
  ...
  facts (`medallia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "medallia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"medallia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "medallia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :medallia-compat/ready)
